...
@section('content')
    <div class="container-fluid">
        ...
        <div class="card shadow mb-4 p-5">
            ...
            <form action="{{ route('students.store') }}" method="POST" class="col-md-12 row" autocomplete="off"
                enctype="multipart/form-data">
                @csrf
                <div class="form-group col-md-6">
                    <label for="studentID">Student ID</label>
                    <input type="text" name="studentID" id="studentID"
                        class="form-control @error('studentID') is-invalid @enderror" placeholder="ITXXXXXXX"
                        value="{{ old('studentID') }}">
                    @error('section')
                ...
                <div class="form-group col-md-6 col-lg-6 col-sm-6">
                    ...
                    <input type="text" name="dob" placeholder="{{ date('d-m-Y') }}" class="form-control datepicker"
                        autocomplete="off" value="{{ old('dob', date('d-m-Y')) }}">
                </div>
                    ...
                    <select class="form-control" id="created_by" name="created_by">
                        @foreach (users() as $key => $user)
                            <option value="{{ $user->name }}"
                                {{ $user->name == auth()->user()->name ? 'selected' : '' }}>{{ $user->name }}</option>
                        @endforeach
                    </select>
                </div> -->
                ...
                <div class="form-group col-md-6 col-lg-6 col-sm-6">
                    ...
                    <input type="text" value="{{ old('nationality') }}" name="nationality" placeholder="-"
                        class="form-control" autocomplete="off">

                </div>
                <div class="form-group col-md-6 col-lg-6 col-sm-6">
                    <label for="name">Race</label>
                    <input type="text" value="{{ old('race') }}" name="religion" placeholder="-"
                        class="form-control" autocomplete="off">
                ...
                <div class="form-group col-md-6 col-lg-6 col-sm-6">
                    ...
                    <input type="number" value="{{ old('phone_1') }}" name="phone_1" placeholder="09XXXXXXXXX"
                        class="form-control @error('phone') is-invalid @enderror" autocomplete="off">
                    @error('phone')
                        <div class="text-danger font-weight-bold">* {{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group col-md-6 col-lg-6 col-sm-6">
                    ...
                    <input type="number" value="{{ old('phone_2') }}" name="phone_2" placeholder="09XXXXXXXXX"
                        class="form-control @error('phone') is-invalid @enderror" autocomplete="off">
                    @error('phone')
                        <div class="text-danger font-weight-bold">* {{ $message }}</div>
                    @enderror
                ...
                <div class="form-group col-md-6 col-sm-6 col-lg-6">
                    ...
                    <textarea class="form-control @error('address') is-invalid @enderror" id="address" name="address"
                        placeholder="Pyinmana">{{ old('address') }}</textarea>
                    @error('address')
                        <div class="text-danger font-weight-bold">* {{ $message }}</div>
                    @enderror
                </div>
