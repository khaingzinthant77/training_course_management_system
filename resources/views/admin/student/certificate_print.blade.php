...
<body>
    ...
    <div class="wrapper">
      ...
      <p style="text-align:center;font-size: 36px;">{{$print_certificate->name}}</p>
      ...
      @foreach($certificate_majors as $key=>$major)
        <p style="text-align:center;font-size: 18px;margin-bottom: 7px;">{{$major->name}}</p>
      @endforeach
       <div style="position:absolute;left: 100px;bottom: 160px;">
         @if($print_option == 1)

        @if($print_certificate->photo != null)
            <img src="{{ asset('uploads/student_photo/' . $print_certificate->photo) }}" style="width:70px;height:70px;" class="img-fluid">
        @else
      ...
      <div style="position: absolute;left: 144px;bottom: 124px;">
        ...
        <img src="{{ asset('uploads/qr_code/qr_' . $print_certificate->studentID.'.png') }}" style="width:45px;height:45px;" class="img-fluid">
      </div>
    </div>
    ...
    <script>
      ...
      // let certificate = @json($print_certificate);
      //   console.log(certificate);
        ...
        window.onafterprint = function() {
    		  if (window.matchMedia('print').matches) {
    			  let certificate = @json($print_certificate);
            var certificate_id = certificate.id;
            var token = '{{csrf_token()}}';
            // console.log(token);
            $.ajax({
                url: "<?php echo route('store_certificate_history'); ?>",
                method: 'POST',
                dataType: 'html',
                data: {
